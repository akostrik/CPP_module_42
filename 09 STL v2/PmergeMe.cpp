...
// 3-11     9-15     2-10     8-14     4-16     1-7     12-13     5-6    // 3 < 11, 9 < 15, ...
// 3-11-9-15         2-10-8-14         1-7-4-16         5-6-12-13        // 3 < 9, 2 < 8, 1 < 4, 5 < 12
// 2-10- 8-14- 3-11- 9-15              1- 7- 4-16- 5- 6-12-13
// 1- 7- 4-16- 5- 6-12-13-2-10- 8-14- 3-11- 9-15
// in the specific order
void PmergeMe::to_2_lists() {
  for(; this->size() > 2 ; )
...
void PmergeMe::reverse(int n, int m) {
...
void PmergeMe::change_order(std::list<unsigned int> *l) {
...
void PmergeMe::to_N_lists() {
  unsigned int min = *(this->begin()->begin());
 ...
 //     if (it != this->begin()) {
      print_list("0)");
      std::cout << *(insert->begin()) << " != " << min << " ?" << std::endl;
      if (*(insert->begin()) != min) {
        print_list("1)");
        change_order(insert);
        print_list("2)");
